...
export function calculateProjectProgress(project: Project, milestones: Milestone[], tasks: Task[]): number {
...
export function getProjectHealth(project: Project, milestones: Milestone[], tasks: Task[]): {
  ...
  if (issues.length > 0) {
    status = overdueMilestones.length > 0 || highPriorityIncomplete.length > 0 ? 'critical' : 'warning'
  }
...
export function getProjectTimelineStatus(project: Project): {
  ...
  if (progressDiff < -20) {
    return { status: 'delayed', message: 'Project is significantly behind schedule' }
...
export function getProjectBudgetStatus(project: Project, actualCost?: number): {
  ...
  if (!actualCost) {
    return { status: 'on-budget', percentage: 0, message: 'No actual costs tracked' }
  }
...
// Get overdue items
export function getOverdueItems(milestones: Milestone[], tasks: Task[]): {
...
export function getProjectStatistics(project: Project, milestones: Milestone[], tasks: Task[]) {
  ...
  const { milestones: overdueMilestones, tasks: overdueTasks } = getOverdueItems(milestones, tasks)
